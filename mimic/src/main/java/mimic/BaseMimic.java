...
import static mimic.BaseMimic.MethodType.*;
...
abstract class BaseMimic<T> extends AbstractMimic<T> {
    ...
    final static Cache<String, Tuple2<MethodType, String>> memo = Cache.build(null, true);

    enum MethodType {
        OTHER, COMMON, GETTER, SETTER, CHAIN_SETTER
    }
    ...
    protected Tuple2<MethodType, String> methodDecide(String name, int length, Method method) {
        ...
        if (method.isDefault() && method.getReturnType() != Void.TYPE && length == 0 && (values.containsKey(name) || values.containsKey(tryDeGetter(name)))) {
            final Tuple2<MethodType, String> r = tuple(GETTER, values.containsKey(name) ? name : tryDeGetter(name));
            memo.put(index, r);
            return r;
        }
        //normal default method
        if (method.isDefault()) {
            final Tuple2<MethodType, String> r = tuple(OTHER, name);
            memo.put(index, r);
            return r;
        }
        //common method //todo hashCode should be store?
        if (CommonMethodName.contains(name)) {
            final Tuple2<MethodType, String> r = tuple(COMMON, name);
            memo.put(index, r);
            return r;
        }
        //java bean getter
        if (length == 0 && name.startsWith("is") && method.getReturnType() != Void.TYPE && method.getReturnType() != type) {
            final Tuple2<MethodType, String> r = tuple(GETTER, name.substring(2));
            memo.put(index, r);
            return r;
        }
        if (length == 0 && name.startsWith("get") && method.getReturnType() != Void.TYPE && method.getReturnType() != type) {
            final Tuple2<MethodType, String> r = tuple(GETTER, name.substring(3));
            memo.put(index, r);
            return r;
        }
        //fluent getter
        if (fluent.get() && length == 0 && method.getReturnType() != Void.TYPE && method.getReturnType() != type) {
            final Tuple2<MethodType, String> r = tuple(GETTER, name);
            memo.put(index, r);
            return r;
        }
        //java bean setter
        if (length == 1 && name.startsWith("set") && method.getReturnType() == Void.TYPE) {
            final Tuple2<MethodType, String> r = tuple(SETTER, name.substring(3));
            memo.put(index, r);
            return r;
        }
        //chain setter
        if (length == 1 && name.startsWith("set") && method.getReturnType() == type) {
            final Tuple2<MethodType, String> r = tuple(CHAIN_SETTER, name.substring(3));
            memo.put(index, r);
            return r;
        }
        //fluent setter
        if (fluent.get() && length == 1 && method.getReturnType() == Void.TYPE) {
            final Tuple2<MethodType, String> r = tuple(SETTER, name);
            memo.put(index, r);
            return r;
        }
        //fluent chain setter
        if (fluent.get() && length == 1 && method.getReturnType() == type) {
            final Tuple2<MethodType, String> r = tuple(CHAIN_SETTER, name);
            memo.put(index, r);
            return r;
        }
        //other
    ...
    public final Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
        ...
        final Tuple2<MethodType, String> decision = methodDecide(name, length, method);
        switch (decision.v1) {
            case GETTER:
